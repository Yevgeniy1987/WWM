...
function showUpdatedMovieModal() {
  const modalForm = `<form data-action= "updateMovie" id="modalForm">
      <div class="flex-col justify-between">
        <div class="modal-window-form-field">
          <label for ="title">Name</label>
        ...
        <div>
  <button class="h-7 w-28 border bg-yellow-400 rounded" type="submit" id="updateBtn">
    Update
  </button>
  <button class="h-7 w-28 border bg-red-400 rounded" type="reset" id="resetBtn">Clear</button>
</div>
...
function updateMovie(form) {
  ...
  const updateMoveBtn = document.getElementById("updateBtn");
  updateMoveBtn.addEventListener("submit", (event) => {
    event.preventDefault();
    const actionBtn = event.target;
    const updateBtn = actionBtn.dataset.action;
     const currentUpdateBtn = event.target;
    const updatingMovieId = Number(currentUpdateBtn.dataset.id);
